...
export default function InvoicePayClient({
  ...
  const handlePay = async () => {
    if (!invoice) return;
    if (!publicKey || !payerAddress) { openWalletModal(true); return; }
    ...
    try {
      ...
      const merchantAta = await getAssociatedTokenAddress(USDC_MINT, merchantPubkey, true);
      ...
      try {
        ...
        if (balance < invoice.total) {
          throw new Error(`Insufficient USDC balance. You have $${balance.toFixed(2)} but need $${invoice.total.toFixed(2)}`);
        }
      } catch (err) {
        if (err instanceof Error && err.message.includes("Insufficient")) throw err;
        throw new Error("USDC token account not found. Please fund your wallet.");
      }

      const amountLamports = BigInt(Math.round(invoice.total * Math.pow(10, USDC_DECIMALS)));
      const transaction = new Transaction();

      try { await getAccount(connection, merchantAta); } catch {
        transaction.add(createAssociatedTokenAccountInstruction(userPubkey, merchantAta, merchantPubkey, USDC_MINT));
      }

      transaction.add(createTransferInstruction(userAta, merchantAta, userPubkey, amountLamports));
      ...
      if (walletSignTransaction) {
        ...
        sig = await sendTransaction!(transaction, connection, { skipPreflight: true });
      }
      ...
      await fetch(`/api/invoices/view/${token}/pay`, {
        ...
        body: JSON.stringify({ paymentSignature: sig, payerWallet: payerAddress }),
      });
  ...
  const explorerUrl = txSignature
    ? `https://explorer.solana.com/tx/${txSignature}${IS_DEVNET ? "?cluster=devnet" : ""}`
    : null;
  ...
  const isOverdue =
    invoice && new Date(invoice.dueDate) < new Date() && !["paid", "cancelled"].includes(invoice.status);
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <header className="border-b border-[#1f1f1f] px-6 py-4">
        <div className="mx-auto max-w-6xl flex items-center justify-between">
          <div className="flex items-center gap-6">
            <Link href="/" className="text-lg font-bold text-[#00ff41] tracking-tight">
              Settlr
            </Link>
            <span className="text-sm font-medium text-white underline underline-offset-4">Payment</span>
            <Link href="/help" className="text-sm text-[#666] hover:text-white transition-colors">
              Help Center
            </Link>
          </div>
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        <AnimatePresence mode="wait">
          {/* Loading */}
          {state === "loading" && (
            <motion.div key="loading" {...fadeIn} className="flex flex-col items-center gap-3 py-20">
          ...
          {/* Not found */}
          {state === "not-found" && (
            <motion.div key="not-found" {...fadeIn} className="max-w-md mx-auto text-center py-20">
              ...
              <p className="mt-2 text-sm text-[#666]">This invoice link is invalid or has expired.</p>
            </motion.div>
          )}

          {/* Cancelled */}
          {state === "cancelled" && invoice && (
            <motion.div key="cancelled" {...fadeIn} className="max-w-md mx-auto text-center py-20">
              <div className="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-full bg-red-500/10">
                <XCircle className="h-8 w-8 text-red-400" />
              </div>
              <h2 className="text-xl font-bold text-white">Invoice Cancelled</h2>
              <p className="mt-2 text-sm text-[#666]">
                Invoice #{invoice.invoiceNumber} from <strong className="text-white">{invoice.merchantName}</strong> has been cancelled.
              </p>
            </motion.div>
          )}
          ...
          {/* Error */}
          {state === "error" && (
            <motion.div key="error" {...fadeIn} className="max-w-md mx-auto text-center py-20">
              <div className="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-full bg-red-500/10">
                <AlertCircle className="h-8 w-8 text-red-400" />
              </div>
              <h2 className="text-xl font-bold text-white">Something Went Wrong</h2>
              <p className="mt-2 text-sm text-[#666]">{error || "Please try again later."}</p>
            </motion.div>
          )}

          {/* Ready / Paying / Success / Already Paid */}
          {(state === "ready" || state === "paying" || state === "success" || state === "already-paid") && invoice && (
            <motion.div key="invoice" {...fadeIn}>
              ...
              <div className="mb-8">
                ...
                <p className="text-sm text-[#888]">
                  From <span className="text-white font-medium">{invoice.merchantName}</span>
                </p>
              </div>
                ...
                <div className="lg:col-span-3 space-y-6">
                  ...
                  <div className="rounded-xl bg-[#141414] border border-[#1f1f1f] p-6">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] text-[#555] uppercase tracking-wider font-semibold">Amount Due</span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-6 pt-4 border-t border-[#1f1f1f]">
                      <div>
                        <span className="text-[10px] text-[#555] uppercase tracking-wider block mb-1">Due Date</span>
                        <span className="text-sm font-medium text-white">{formatDate(invoice.dueDate)}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-[#555] uppercase tracking-wider block mb-1">Status</span>
                        <span className={`text-sm font-medium flex items-center gap-1.5 ${
                          state === "success" || state === "already-paid" ? "text-[#00ff41]" :
                          isOverdue ? "text-amber-400" : "text-[#00ff41]"
                        }`}>
                          <span className={`h-1.5 w-1.5 rounded-full ${
                            state === "success" || state === "already-paid" ? "bg-[#00ff41]" :
                            isOverdue ? "bg-amber-400" : "bg-[#00ff41]"
                          }`} />
                          {state === "success" ? "Paid" :
                           state === "already-paid" ? "Paid" :
                           isOverdue ? "Overdue" : "Pending Payment"}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Line Items */}
                  <div className="rounded-xl bg-[#141414] border border-[#1f1f1f] p-6">
                    <span className="text-[10px] text-[#555] uppercase tracking-wider font-semibold block mb-4">Line Items</span>
                  ...
                  {(state === "success" || state === "already-paid") && txSignature && (
                    <div className="rounded-xl bg-[#00ff41]/5 border border-[#00ff41]/20 p-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-semibold text-[#00ff41]">
                          {state === "success" ? "Payment Successful : "This invoice has been paid"}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 rounded-lg bg-[#1a1a1a] border border-[#333] px-3 py-2">
                        <code className="flex-1 truncate text-xs text-[#aaa] font-mono">{txSignature}</code>
                        <button onClick={copySignature} className="text-[#00ff41] hover:text-[#00dd38] transition-colors">
                          {copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
                        </button>
                        {explorerUrl && (
                          <a href={explorerUrl} target="_blank" rel="noopener noreferrer" className="text-[#00ff41] hover:text-[#00dd38]">
                            <ExternalLink className="h-3.5 w-3.5" />
                          </a>
                        )}
                      </div>
                      {invoice.paidAt && (
                        <p className="text-center text-xs text-[#666] mt-2">Paid on {formatDate(invoice.paidAt)}</p>
                      )}
                    </div>
                  )}
                </div>
                  ...
                  {(state === "ready" || state === "paying") && (
                    <>
                      ...
                      <div className="rounded-xl bg-[#141414] border border-[#1f1f1f] p-5">
                        <h3 className="text-sm font-semibold text-white mb-4">Select Asset</h3>
                          ...
                          {ASSETS.map((asset, i) => (
                            ...
                            >
                              <div className={`h-8 w-8 rounded-full flex items-center justify-center text-sm font-bold ${
                                selectedAsset === i ? "bg-[#00ff41] text-black" : "bg-[#333] text-[#888]"
                              }`}>
                                {asset.icon}
                              </div>
                              <div className="flex-1">
                                <div className="text-sm font-semibold text-white">{asset.symbol}</div>
                                <div className="text-[11px] text-[#666]">{asset.network}</div>
                              </div>
                      ...
                      <div>
                        ...
                        {connecting ? (
                          <div className="flex items-center justify-center gap-2 py-4">
                            ...
                            <span className="text-sm text-[#666]">Initializing wallet...</span>
                          </div>
                        ) : !connected ? (
                          <button
                          ...
                          >
                            {state === "paying" ? (
                              ...
                              <>
                                Pay ${invoice.total.toLocaleString("en-US", { minimumFractionDigits: 2 })}
                      ...
                      <div className="rounded-xl bg-[#141414] border border-[#1f1f1f] p-5">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-sm font-semibold text-white">Network Fees</span>
                        </div>
                        <p className="text-xs text-[#666] leading-relaxed">
                          Payments are processed instantly on-chain. Gas fees are calculated at execution.
                          Ensure your wallet has sufficient balance for network overhead.
                        </p>
                      </div>
                    </>
                  )}
              ...
              <div className="flex items-center justify-between py-6 border-t border-[#1f1f1f] mt-4 text-[10px] text-[#444] uppercase tracking-wider">
                ...
                <span>System Time: {new Date().toISOString().replace("T", " ").slice(0, 19)} UTC</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
